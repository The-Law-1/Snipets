...
def get_snippets_service() -> GetSnippets:
    return GetSnippets()
...
async def create_snippet(
    snippet: CreateSnippet, saveSnippet: SaveSnippet = Depends(get_save_snippet_service)
) -> Any:
    try:
...
async def get_snippets(
    ...
    try:
        ...
        serialized_snippets = [
            SerializedSnippet(**snippet.__dict__) for snippet in result
        ]
...
async def delete_snippet(
    snippet_id: str, deleteSnippet: DeleteSnippet = Depends(get_delete_snippet_service)
) -> Any:
    try:
